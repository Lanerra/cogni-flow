...
class WorkingMemory:
    ...
    def __init__(self, capacity: int = 5, embedding_model: str = "all-MiniLM-L6-v2"):
        ...
        try:
            ...
            print(
                f"Error loading embedding model: {e}. Falling back to text-based similarity."
            )
    ...
    def update(self, new_elements: List[Dict], max_age: int = 10):
        ...
        if current_indices:
            ...
            self.embeddings = (
                [self.embeddings[i] for i in current_indices] if self.embeddings else []
            )
            ...
            if self.elements:
                for idx, existing in enumerate(self.elements):
                    ...
                    if similarity > 0.8:  # Threshold for considering elements similar
                        ...
                        self.importance[idx] = max(
                            self.importance[idx], element.get("importance", 0.5)
                        )
                        self.recency[idx] = 0
                        found_similar = True
                        break
        ...
        if len(self.elements) > self.capacity:
            ...
            activation = [
                imp * (1 / (rec + 1)) for imp, rec in zip(self.importance, self.recency)
            ]
    ...
    def get_active_elements(
        ...
        if context:
            if self.use_embeddings:
                ...
                if context_text:
                    context_embedding = self._generate_embedding(
                    ...
                    context_relevance = [
                        self._contextual_relevance(e, context) for e in self.elements
                    ]
            else:
...
class MetaCognitiveController:
    ...
    def __init__(self):
        ...
        self.cognitive_state = (
            "neutral"  # Current cognitive state (explore, focus, etc.)
        )
    ...
    def evaluate_output(self, output: Dict, analysis: Dict, target_state: Dict) -> Dict:
        ...
        self.generation_history.append(
            {"output": output, "analysis": analysis, "target_state": target_state}
        )
        ...
        if target_metrics:
            ...
            self.goal_alignment = (
                sum(differences) / len(differences) if differences else 0.5
            )
        else:
    ...
    def _update_cognitive_state(self):
        """Update cognitive state with more sensitive thresholds."""
        # Make thresholds more sensitive
        if self.goal_alignment < 0.4:  # Increased from 0.3
            self.cognitive_state = "explore"
        # If somewhat off-target, make minor shift
        elif self.goal_alignment < 0.7:  # Increased from 0.6
            self.cognitive_state = "shift"
        # If good alignment but poor quality, focus to improve
        elif self.goal_alignment > 0.6:  # Decreased from 0.7
            self.cognitive_state = "focus"
        # Add progressive weighting to avoid neutral bias
        elif (
            random.random() < 0.3
        ):  # 30% chance to shift state even when neutral is indicated
            # Choose a slight shift based on current context
            self.cognitive_state = random.choice(["explore", "shift", "focus"])
        else:
        ...
        if len(self.generation_history) >= 3:
            recent_states = [
                entry.get("cognitive_state", "neutral")
                for entry in self.generation_history[-3:]
            ]
...
class EnhancedMetaCognitiveSystem:
    ...
    def __init__(
        ...
        try:
            ...
            print(
                f"Error loading sentiment model: {e}. Using fallback sentiment analysis."
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.model not in available_models:
                    print(
                        f"Warning: Model '{self.model}' not found in Ollama. Available models: {available_models}"
                    )
            else:
                print(
                    f"Warning: Could not verify models. Ollama returned status code {response.status_code}"
                )
        except Exception as e:
            print(f"Warning: Could not connect to Ollama: {e}")

    def generate_text(
        self, prompt: str, narrative_style: str = "standard", max_paragraphs: int = 5
    ) -> str:
        """
        ...
        for i in range(max_paragraphs):
            ...
            if self._detect_repetition(paragraph, full_response.split("\n\n")):
                print("Repetition detected, adjusting parameters to break pattern")
                # Apply disruption technique
                self._apply_profile_weighted(
                    "creative", 0.8
                )  # Boost creativity temporarily
                self.current_params[
                    "presence_penalty"
                ] += 0.3  # Increase presence penalty
            ...
            paragraph_elements = self._extract_memory_elements(
                paragraph, analysis_results
            )
            ...
            meta_evaluation = self.meta_cognitive.evaluate_output(
                {"text": paragraph}, analysis_results, target_state
            )
    ...
    def _generate_paragraph(self, context: str) -> str:
        ...
        try:
            ...
            if active_elements:
                ...
                memory_prompt = " Remember these key elements: " + "; ".join(
                    memory_concepts
                )
    ...
    def _extract_memory_elements(self, paragraph: str, analysis: Dict) -> List[Dict]:
        ...
        # Extract emotion
        if (
            "emotionality" in analysis
            and "dominant_emotion" in analysis["emotionality"]
        ):
    ...
    def _apply_metacognitive_adjustments(self, evaluation: Dict):
        """Apply parameter adjustments with damping to prevent drift."""
        if "parameter_adjustments" in evaluation:
            ...
            for param, adjustment in adjustments.items():
                if param in self.current_params:
                    # Calculate distance from neutral/default value
                    param_defaults = {
                        "temperature": 0.7,
                        "top_p": 0.9,
                        "top_k": 40,
                    ...
                    default_val = param_defaults.get(param, 0.5)
                    current_val = self.current_params[param]
                    distance_from_default = abs(current_val - default_val)

                    # Apply damping factor based on distance
                    damping_factor = max(0.1, 1.0 - distance_from_default)
                    damped_adjustment = adjustment * damping_factor

                    # Apply damped adjustment
                    self.current_params[param] += damped_adjustment

                    # Ensure parameters stay in reasonable ranges
                    if param == "temperature":
                        self.current_params[param] = max(
                            0.1, min(1.5, self.current_params[param])
                        )
                    elif param == "top_p":
                        self.current_params[param] = max(
                            0.5, min(1.0, self.current_params[param])
                        )
                    elif param == "top_k":
                        self.current_params[param] = max(
                            5, min(100, self.current_params[param])
                        )
                    elif param in ["frequency_penalty", "presence_penalty"]:
                        self.current_params[param] = max(
                            0.0, min(2.0, self.current_params[param])
                        )

    def _detect_repetition(
        self, new_paragraph: str, previous_paragraphs: List[str], threshold: float = 0.7
    ) -> bool:
        """
        Detect if the new paragraph contains significant repetition of content from previous paragraphs.

        Args:
            new_paragraph: The newly generated paragraph
            previous_paragraphs: List of previously generated paragraphs
            threshold: Similarity threshold for repetition detection

        Returns:
            Boolean indicating if repetition was detected
        """
        if not previous_paragraphs or not new_paragraph:
            return False

        # Clean and tokenize the new paragraph
        new_tokens = set(re.findall(r"\b\w+\b", new_paragraph.lower()))
        if not new_tokens:
            return False

        # Check for n-gram overlaps
        for prev_para in previous_paragraphs[-3:]:  # Check last 3 paragraphs
            prev_tokens = set(re.findall(r"\b\w+\b", prev_para.lower()))
            if not prev_tokens:
                continue

            # Calculate Jaccard similarity
            overlap = len(new_tokens.intersection(prev_tokens))
            union = len(new_tokens.union(prev_tokens))
            ...
            new_phrases = self._extract_phrases(new_paragraph)
            prev_phrases = self._extract_phrases(prev_para)
    ...
    def _analyze_creativity(self, text: str) -> Dict:
        ...
        figurative_count = sum(
            1 for marker in figurative_markers if marker in text_lower
        )
        ...
        ]  # Only consider words > 3 chars
        unusual_word_ratio = sum(
            1 for word in word_list if word not in common_words
        ) / max(1, len(word_list))
    ...
    def _analyze_formality(self, text: str) -> Dict:
        ...
        contraction_count = sum(
            1 for contraction in contractions if contraction in text_lower
        )

        # Check for first/second person pronouns (informal)
        first_second_person = len(
            re.findall(
        ...
        formality_base = (
            formal_count * 2 - informal_count - contraction_count / 2
        ) / max(5, len(words) / 20)
    ...
    def _analyze_narrative_tension(self, text: str) -> Dict:
        ...
        punctuation_factor = (exclamations + questions + ellipses * 0.5) / max(
            2, len(sentences)
        )
    ...
    def _analyze_dialogue_content(self, text: str) -> Dict:
        ...
        dialogue_tags = re.findall(
            r"(said|asked|replied|whispered|shouted|exclaimed|muttered|responded)",
            text.lower(),
        )
    ...
    def _analyze_emotional_intensity(self, text: str) -> Dict:
        ...
        dominant_emotion = (
            ...
            if emotion_counts and max(emotion_counts.values()) > 0
            else "neutral"
        )
    ...
    def _is_conclusion(self, paragraph: str) -> bool:
        ...
        has_conclusion_marker = any(
            indicator in paragraph_lower for indicator in conclusion_indicators
        )
        ...
        return (
            ...
            or (is_short_final and in_resolution_phase)
            or has_resolution_feel
        )
...
if __name__ == "__main__":
    ...
    narrative_response = system.generate_text(
        narrative_prompt, narrative_style="building_tension", max_paragraphs=30
    )
